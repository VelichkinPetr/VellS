from __future__ import annotations

import aiosqlite

from buisness_logic.db import Chapters


class ChaptersRepo:

    def __init__(self, db_path: str) -> None:
        self.db_path = db_path

    async def init_table(self) -> None:

        sql_command = '''
                    CREATE TABLE IF NOT EXISTS `Chapters`(
                        `id` INTEGER PRIMARY KEY AUTOINCREMENT,
                        `name` TEXT NOT NULL,
                        `subject_id` INTEGER,
                        `created_at` TEXT DEFAULT CURRENT_TIMESTAMP,

                        CONSTRAINT `chapters_subjects` 
                        FOREIGN KEY (`subject_id`)
                        REFERENCES Subjects(`id`));
                        '''
        async with aiosqlite.connect(self.db_path) as db:
            await db.executescript(sql_command)
            await db.commit()

    async def create_chapter(self, sub_id: int, chapter_name: str) -> None:

        sql_command = '''INSERT INTO `Chapters`(`name`, `subject_id`)
                        VALUES (:chapter_name, :sub_id)'''
        async with (aiosqlite.connect(self.db_path) as db):
            db.row_factory = aiosqlite.Row
            data = ({"sub_id": sub_id,
                     "chapter_name": chapter_name})

            await db.execute(sql_command, data)
            await db.commit()

    #async def search_topic(self, topic_text: str):
    #    sql_command = '''SELECT * FROM `Topics` WHERE `text` = :topic_text'''
#
    #    data = ({"topic_text": topic_text})
#
    #    async with aiosqlite.connect(self.db_path) as db:
    #        db.row_factory = aiosqlite.Row
#
    #        cursor = await db.execute(sql_command, data)
    #        raw = await cursor.fetchone()
#
    #        if raw is not None:
    #            topic = Topics(**dict(raw))
    #            if topic.text == topic_text:
    #                return True
    #        return False

    async def fetch_chapters(self, sub_id: int) -> list[Chapters] | None:

        sql_command = """
                        SELECT `id`, `name`, `subject_id`, `created_at` FROM `Chapters`
                        WHERE `subject_id` = ?;
                        """
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute(sql_command, [sub_id])
            all_topics = await cursor.fetchall()

            if all_topics is not None:
                all_topics = [Chapters(*topic) for topic in all_topics]
            return all_topics

    async def remove_chapter(self, sub_id: int, chapter_name: str) -> None:

        sql_command = """
                       DELETE FROM `Chapters`
                       WHERE `subject_id` = ? AND `name` = ?;
                      """
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(sql_command, [sub_id, chapter_name])
            await db.commit()
